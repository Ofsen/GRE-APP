...
import { Text, View, ScrollView, Modal, StyleSheet, Dimensions, TouchableOpacity, Alert } from 'react-native';
// Redux
...
const Orders = () => {
	...
	const validerCommande = () => {
		if (count === 0)
			axios
				...
				.then((data) => {
					dispatch(setOrder(data.data.count));
				})
				.catch((err) => {
					console.log(err);
				});
	...
	const validateOrder = () =>
		Alert.alert('Valider votre commande', 'Êtes vous sûrs de votre commande?', [
			{
				text: 'Annuler',
				onPress: () => console.log('Order canceled'),
				style: 'cancel',
			},
			{ text: 'OK', onPress: () => validerCommande() },
		]);
